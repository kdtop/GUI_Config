object frmAddRemDef: TfrmAddRemDef
  Left = 0
  Top = 0
  Caption = 'frmAddRemDef'
  ClientHeight = 350
  ClientWidth = 382
  Color = clBtnFace
  Constraints.MinHeight = 380
  Constraints.MinWidth = 390
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlbottom: TPanel
    Left = 0
    Top = 310
    Width = 382
    Height = 40
    Align = alBottom
    TabOrder = 0
    DesignSize = (
      382
      40)
    object btnCancel: TBitBtn
      Left = 5
      Top = 5
      Width = 75
      Height = 30
      Anchors = [akLeft, akTop, akBottom]
      Caption = '&Cancel'
      ModalResult = 2
      TabOrder = 3
      Glyph.Data = {
        
        11B2050FB10107A0000188FF00FFFF00FFFF00FFFF00FFFF00FF}
    end
    object btnDone: TBitBtn
      Left = 303
      Top = 6
      Width = 75
      Height = 30
      Anchors = [akTop, akRight, akBottom]
      Caption = '&Done'
      Enabled = False
      ModalResult = 1
      TabOrder = 2
      Glyph.Data = {
        
        0303}
      NumGlyphs = 2
    end
    object btnPrev: TBitBtn
      Left = 99
      Top = 6
      Width = 90
      Height = 30
      Caption = '&Previous'
      TabOrder = 1
      Visible = False
      Glyph.Data = {
        
        ECFED8E9ECFED6E5E8FFD8E9ECFED8E9ECFED7E8EBFFD8E9ECFE}
    end
    object btnNext: TBitBtn
      Left = 195
      Top = 6
      Width = 90
      Height = 30
      Caption = '&Next'
      Default = True
      TabOrder = 0
      Visible = False
      Glyph.Data = {
        
        ECFED8E9ECFED6E5E8FFD8E9ECFED8E9ECFED7E8EBFFD8E9ECFE}
      Layout = blGlyphRight
    end
  end
  object pnlTop: TPanel
    Left = 0
    Top = 0
    Width = 382
    Height = 49
    Align = alTop
    TabOrder = 1
    object Image1: TImage
      Left = 5
      Top = 5
      Width = 35
      Height = 35
      Picture.Data = {
        
        EDFF}
    end
    object lblAddRemDef: TLabel
      Left = 56
      Top = 14
      Width = 201
      Height = 19
      Caption = 'Add New Reminder Definition'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Times New Roman'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btnStoreEdit: TBitBtn
      Left = 319
      Top = 8
      Width = 50
      Height = 35
      TabOrder = 0
      Visible = False
      Glyph.Data = {
        
        16FFB78216FFB88317FFC0A058FFD4E5E9FFD8E9EDFFD8E9EDFF}
    end
  end
  object pcWizardPageControl: TPageControl
    Left = 0
    Top = 49
    Width = 382
    Height = 261
    ActivePage = tsNewFinding
    Align = alClient
    TabOrder = 2
    object tsNewFinding: TTabSheet
      Caption = 'New Definition'
      ImageIndex = 2
      DesignSize = (
        374
        233)
      object Label2: TLabel
        Left = 37
        Top = 19
        Width = 37
        Height = 16
        Caption = 'Name'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 27
        Top = 71
        Width = 47
        Height = 16
        Caption = 'Gender'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 41
        Top = 98
        Width = 33
        Height = 16
        Caption = 'Class'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label1: TLabel
        Left = 28
        Top = 126
        Width = 46
        Height = 13
        Alignment = taRightJustify
        Caption = 'Sponsor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        WordWrap = True
      end
      object Label3: TLabel
        Left = 39
        Top = 153
        Width = 35
        Height = 13
        Alignment = taRightJustify
        Caption = 'Usage'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        WordWrap = True
      end
      object Label4: TLabel
        Left = 3
        Top = 45
        Width = 73
        Height = 14
        Caption = 'Display Name'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        WordWrap = True
      end
      object edtName: TEdit
        Left = 80
        Top = 16
        Width = 281
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        Color = clYellow
        TabOrder = 0
        OnChange = edtInputChange
      end
      object btnCreateItem: TBitBtn
        Left = 52
        Top = 181
        Width = 309
        Height = 41
        Anchors = [akLeft, akBottom]
        Caption = '&Create Reminder Definition, then Edit Details'
        Enabled = False
        TabOrder = 6
        OnClick = btnCreateItemClick
        Glyph.Data = {
          
          EDFFD8E9EDFFD8E9EDFFD8E9EDFFD8E9EDFFD8E9EDFFD8E9EDFF}
      end
      object cboGender: TComboBox
        Left = 80
        Top = 70
        Width = 171
        Height = 21
        Color = clWhite
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 2
        Text = 'Male or Female Patients'
        Items.Strings = (
          'Male or Female Patients'
          'Male Patients Only'
          'Female Patients Only')
      end
      object cboClass: TComboBox
        Left = 80
        Top = 97
        Width = 281
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        Color = clWhite
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 3
        Text = 'LOCAL'
        OnChange = edtInputChange
        Items.Strings = (
          'LOCAL'
          'NATIONAL'
          'VISN')
      end
      object cboSponsor: TORComboBox
        Left = 80
        Top = 124
        Width = 281
        Height = 21
        Hint = 'Enter name,Full SSN ,Last 4 (x1234),'#39'HRN'#39',DOB, or Phone#'
        Anchors = [akLeft, akTop, akRight, akBottom]
        CaseChanged = False
        Style = orcsDropDown
        AutoSelect = True
        Caption = 'Patient'
        Color = clYellow
        DropDownCount = 8
        ItemHeight = 13
        ItemTipColor = clWindow
        ItemTipEnable = True
        ListItemsOnly = False
        LongList = True
        LookupPiece = 2
        MaxLength = 0
        ParentShowHint = False
        Pieces = '2,3'
        ShowHint = True
        Sorted = False
        SynonymChars = '<>'
        TabPositions = '20,25,30,60'
        TabOrder = 4
        OnChange = cboSponsorChange
        OnClick = cboSponsorClick
        OnDblClick = cboSponsorDblClick
        OnDropDown = cboSponsorDropDown
        OnNeedData = cboSponsorNeedData
        CharsNeedMatch = 1
      end
      object edtUsage: TEdit
        Left = 80
        Top = 151
        Width = 281
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        Color = clYellow
        TabOrder = 5
        OnChange = edtInputChange
        OnClick = edtUsageClick
        OnEnter = edtUsageClick
      end
      object edtDisplayName: TEdit
        Left = 80
        Top = 43
        Width = 281
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        Color = clYellow
        TabOrder = 1
        OnChange = edtInputChange
      end
    end
  end
end
