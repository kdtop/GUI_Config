...
uses
  ...
  TypesU, GlobalsU, SortStringGrid;
...
function  PostChanges(Grid : TSortStringGrid; SilentMode : boolean=true; AddingNewRecord : boolean = false) : TModalResult;
...
implementation
  ...
  uses  UtilU, rRPCsU, EditTextU, fMultiRecEditU, PostU, IniFiles,
      ...
      if tempFile = FileNum then begin //handle subfiles later...
        ...
        if Pos('W',fieldDef)>0 then begin
          ...
          if IsWPField(CachedWPField, FileNum,fieldNum) then begin
            ...
            EditWPTextForm := TEditWPTextForm.Create(nil);
  ...
  begin
    ...
    if BasicMode=false then begin
      ...
      for i := 0 to CurrentData.Count-1 do begin
        ...
        //Application.processmessages; //kt temp.
      end;
  ...
  function PostChanges(Grid : TSortStringGrid; SilentMode : boolean; AddingNewRecord : boolean) : TModalResult;
    ...
    try
      ...
      if Changes.Count>0 then begin
        if AddingNewRecord then begin
          ...
          Name := UtilU.GetOneFldValue(Changes, GridInfo.FileNum, '.01', 5);
    ...
    case FieldType of
      ...
      fmftDate:  //Date
        begin
          ...
          AResult := EditDateTimeForm.ShowModal;
          ...
          end else if AResult = mrNo then begin  //Signal for None
        ...
        //function (InitValue, FileNum, IENS, FieldNum : string; Changed, CanSelect : boolean; ExtraInfo : TStringList) : string
      ...
      fmftPtr:   //Pointer to file.
        begin
          ...
          {
          TargetFileNum := ExtractNum (FieldDef,Pos('P',FieldDef)+1);
          FieldLookupForm:= TFieldLookupForm.Create(Nil);
          FieldLookupForm.PrepForm(TargetFileNum, Grid.Cells[ACol,ARow], FileNum, FieldNum);
          FieldLookupForm.Auto_Press_Edit_Button := AutoPressEditButtonInDetailDialog;
      ...
      fmftVarPtr:  //Variable Pointer to file.
        begin
          ...
          GridInfo.ExtractVarPtrInfo(FieldNum, VarPtrInfo);
          FieldLookupForm:= TFieldLookupForm.Create(Nil);
          FieldLookupForm.PrepFormAsMultFile(VarPtrInfo,Grid.Cells[ACol,ARow], FileNum, FieldNum);
          FieldLookupForm.Auto_Press_Edit_Button := AutoPressEditButtonInDetailDialog;
          VarPtrInfo.Free;
          if FieldLookupForm.ShowModal = mrOK then begin
            Grid.Cells[ACol,ARow] := FieldLookupForm.SelectedValue;
      ...
      fmftNumber:  //Numeric value
        begin
          ...
          EditNumber.NumToEdit.Text := Grid.Cells[ACol,ARow];
          EditNumber.Caption := 'Edit: ' + Grid.Cells[ACol-1,ARow];
          if EditNumber.ShowModal = mrOK then begin
            Grid.Cells[ACol,ARow] := EditNumber.NumToEdit.Text;
          end;
          }
        end;
      ...
      fmftWP:  //Word Processor field
        begin
          ...
          GridInfo.DisplayRefreshIndicated := EditTextForm.ChangesMade;
          }
        end;
      ...
      fmftSubfile: //Subfiles.
        begin
          ...
          if IENS <> '' then begin
            ...
            Grid.Cells[ACol,ARow] := fMultiRecEditU.GetPreviewText(SubFileNum, IENS);
  ...
  begin
    Result := InitValue;
    TargetFileNum := piece(ExtraInfo, EXTRA_INFO_PIECE_DIV, 1);
    tempS := piece(ExtraInfo, EXTRA_INFO_PIECE_DIV, 2);  //boolean
    FieldLookupForm:= TFieldLookupForm.Create(Nil);
  ...
  begin
    ...
    FieldDef := piece(ExtraInfo, EXTRA_INFO_PIECE_DIV, 1);
    FldName :=piece(ExtraInfo, EXTRA_INFO_PIECE_DIV, 2);
    EditNumber.Initialize(FieldDef);
    ...
    if GridInfo.IENS <> '0,' then begin
      ...
      if temp = true then begin
        ...
        for i := 0 to GridList.Count - 1 do begin
          ...
          //kt Application.ProcessMessages; //kt temp
        end;
      end;
    end;
  end;
