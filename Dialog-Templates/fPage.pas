...
uses
  ...
  Dialogs, ExtCtrls, uConst,
  rOrders {, fBase508Form, VA508AccessibilityManager};

type
  //kt TfrmPage = class(TfrmBase508Form)
...
begin
  ...
  Position := poDefault;
  //kt BorderIcons := [];
  //kt BorderStyle := bsNone;
  HandleNeeded;
  //kt SetBounds(0, 0, Width, Height);
end;
...
//CQ12232   FocusFirstControl;
  ...
  Inc(FDisplayCount);
  Inc(FPatientCount);
  //kt FCallingContext := frmFrame.ChangeSource;
...
initialization
  //kt SpecifyFormIsNotADialog(TfrmPage);
