...
function MissingProviderInfo(PCEEdit: TPCEData; PCEType: TPCEType = ptEncounter): boolean;
begin
  if(PCEEdit.Empty and (PCEEdit.Location <> Encounter.Location) and (not Encounter.NeedVisit)) then begin
    PCEEdit.UseEncounter := TRUE;
  end;
  //kt Result := NoPrimaryPCEProvider(PCEEdit.Providers, PCEEdit);
  Result := false; //kt
...
begin  //TPCEData.OK2SignNote: boolean;
  ...
  if(Ask = apNever) or (Ask = apDisable) then begin
    Result := TRUE
  end else begin
    DoSave := FALSE;
    try
      ...
      repeat
        ...
        if(First) then
        begin
          if Needed and (not Outpatient) then begin
            OutPatient := TRUE;
          end;
          if((Ask = apPrimaryAlways) or Needed or
             ((Ask = apPrimaryOutpatient) and Outpatient)) then begin
            if(Providers.PrimaryIdx < 0) then begin
              ...
              if(DoSave and (FProviders.PendingIEN(FALSE) <> 0) and
                (FProviders.IndexOfProvider(IntToStr(FProviders.PendingIEN(FALSE))) < 0)) then begin
        ...
        if(DoAsk) then begin
          if(Asked and ((not Needed) or (not Primary))) then begin
            exit;
          end;
...
begin
  Result := false; exit; //kt added!
  if(FProviders.PrimaryIdx < 0) then begin
    Result := AutoCheckout(FEncLocation);
    if not Result then begin
      for i := 0 to FDiagnoses.Count - 1 do begin
        if not TPCEDiag(FDiagnoses[i]).FDelete then begin
          Result := TRUE;
          break;
        end;
      end;
    ...
    if not Result then begin
      for i := 0 to FProcedures.Count - 1 do begin
        if not TPCEProc(FProcedures[i]).FDelete then begin
          Result := TRUE;
          break;
        end;
      end;
    ...
    if not Result then begin
      for i := 0 to FProviders.Count - 1 do begin
        if not FProviders[i].Delete then begin
          Result := TRUE;
          break;
        end;
      end;
    ...
    if not Result then begin
      ...
      try
        GetHasCPTList(TmpLst);
        if(DataHasCPTCodes(TmpLst)) then begin
          Result := TRUE;
        end;
      finally
        TmpLst.Free;
