...
type
  //kt TVA508RegistrationScreenReader = class(TVA508ScreenReader);
...
var
    TVAGlobalComponentRegistry_FActive: boolean;  //kt moved here
    TVAGlobalComponentRegistry_FGetMsgHookHandle: HHOOK;  //kt moved here
...
begin
  ...
  //kt VA508ComponentCreationCheck(Self, AOwner, FALSE, FALSE);
  CreateGlobalVars;
...
begin
  //kt VA508ComponentDestructionCheck(Self);
...
var
  ...
  procedure error;
  begin
    //kt raise EVA508AccessibilityException.Create(INVALID_COMPONENT_ERROR);
    raise Exception.Create(INVALID_COMPONENT_ERROR);
  end;
...
procedure RegisterAlternateHandleComponent(ComponentClass: TWinControlClass;
                                           AlternateHandleFunc: TVA508AlternateHandleFunc);
//kt var
  //kt data: TAlternateHandleData;
  //kt i: integer;
begin
  //kt if not ScreenReaderSystemActive then exit;
  exit; //kt
  {  //kt
...
procedure RegisterComplexComponentManager(Manager: TVA508ComplexComponentManager);
//kt var
  //kt data: TVA508ComplexComponentManager;
  //kt i: integer;
begin
  {//kt
  if ScreenReaderSystemActive then
...
procedure RegisterManagedComponentClass(Manager: TVA508ManagedComponentClass);
//kt var
  //kt data: TVA508ManagedComponentClass;
  //kt i: integer;
begin
  {//kt
  if ScreenReaderSystemActive then
...
var
  ...
  begin
    Result := TRUE;
    //kt if not assigned(SRComponentData) then exit;
    exit; //kt
    {
...
begin
  ...
  Result := CallNextHookEx(TVAGlobalComponentRegistry_FGetMsgHookHandle, Code, wParam, lParam);
end;
...
begin
  ...
  FComponentData := NewComponentData;
  //kt FGetMsgHookHandle := SetWindowsHookEx(WH_GETMESSAGE, GetMessageHookProc, 0, GetCurrentThreadID);
  TVAGlobalComponentRegistry_FGetMsgHookHandle := SetWindowsHookEx(WH_GETMESSAGE, GetMessageHookProc, 0, GetCurrentThreadID);  //kt
  //kt TVA508RegistrationScreenReader(GetScreenReader).
  //kt            AddComponentDataNeededEventHandler(ComponentDataNeededEvent);
  //ktFActive := TRUE;
  TVAGlobalComponentRegistry_FActive := TRUE; //kt
end;
...
begin
  FDestroying := TRUE;
  //kt FActive := FALSE;
  TVAGlobalComponentRegistry_FActive := FALSE;  //kt
  //kt TVA508RegistrationScreenReader(GetScreenReader).RemoveComponentDataNeededEventHandler(ComponentDataNeededEvent);
  UnhookWindowsHookEx(TVAGlobalComponentRegistry_FGetMsgHookHandle); //kt mod
...
begin
  inherited Create(AOwner);
  //kt VA508ComponentCreationCheck(Self, AOwner, FALSE, TRUE);
  CreateGlobalRegistry;
end;
