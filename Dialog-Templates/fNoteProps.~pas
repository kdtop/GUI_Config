...
type
  ...
  //kt TfrmNoteProperties = class(TfrmBase508Form)
        ...
        begin
          case FCallingTab of
           ...
           CT_NOTES:      begin
                            ...
                            if ANote.IsNewNote then begin
                              //kt GetUnresolvedConsultsInfo; // v26.5 (RV) removed nag screen
                            end;
                            cboNewTitle.InitLongList('');
                            cboAuthor.InitLongList(ANote.AuthorName);
                            if ANote.Author > 0 then cboAuthor.SelectByIEN(ANote.Author);
  ...
  with cboNewTitle do
    ...
    case FCallingTab of
      ...
      CT_NOTES   :  begin            // v26.5 (RV) main changes here
                      ...
                      SetGenericFormSize;
                      //kt ShowRequestList(WantsToCompleteConsult or ConsultTitle);
                      //kt ShowSurgCaseList(IsSurgeryTitle(cboNewTitle.ItemIEN));
...
{ TODO -oRich V. -cSurgery/TIU : Disallow new surgery notes here - MUST be business rule for "BE ENTERED": }
  ...
  if cboCosigner.Visible then begin
  ...
  {
  if FIsClinProcNote then begin
    if (FCPStatusFlag = CP_INSTR_INCOMPLETE) then begin
      ...
      if (calProcDateTime.FMDateTime > 0) then begin
...
begin
  ...
  //kt with uUnresolvedConsults do if (UnresolvedConsultsExist and ShowNagScreen) then pnlConsults.Visible := TRUE;  //v26.27 (RV)
