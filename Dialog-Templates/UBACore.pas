...
interface
uses
  Classes, ORNet, uConst, ORFn, Sysutils, Dialogs, Windows,Messages, UBAGlobals,
  Trpcb {, fFrame};
...
//kt procedure rpcSaveBillingDxEntered;  // save dx enteries regardless of being mandatory....
function  rpcNonBillableOrders(pOrderList: TStringList): TStringList;
//kt function  rpcOrderRequiresDx(pList: TStringList):boolean;
...
//kt procedure rpcBuildSCIEList(pOrderList: TList);
...
//kt procedure SaveBillingData(pBillingData:TStringList);
//kt function  OrdersHaveDx(pOrderList:TStringList):boolean;
procedure SetTreatmentFactors(TFactors: string);
//kt function  AttachDxToOrderList(pOrderList:TStringList):TStringList;
procedure AttachPLTFactorsToDx(var Dest:String;ProblemRec:string);
procedure BALoadStsFlagsAsIs(StsFlagsIN: string);
//ktfunction  BADxEntered:boolean;  //  main logic to determine if dx has been entered for order that requires dx
...
//kt procedure CompleteUnsignedBillingInfo(pOrderList: TStringList);
//kt procedure BuildSaveUnsignedList(pOrderList: TStringList);
...
//kt procedure ClearSelectedOrderDiagnoses(pOrderIDList: TStringList);
procedure LoadConsultOrderRec(var thisRetVal: TBAConsultOrderRec; pOrderID: String; pDxList: TStringList);
//kt procedure CompleteConsultOrderRec(pOrderID: string; pDxList: TStringList);
  ...
  try
     ...
     begin
         ...
         begin
            ...
            //kt frmReview.lstReview.Refresh;
         end;
     end;
  except
     on EListError do
...
begin
   ...
   //kt FastAssign(UBAGlobals.BACopiedOrderFlags, holdList);
...
begin
   ...
   begin
      ...
      //kt FastAssign(UBAGlobals.BAConsultPLFlags, holdList);
