...
//kt function  CountSelectedOrders(const Caller: smallint) : smallint;
//kt function  CompareOrderDx(const Caller: smallint) : boolean;
//kt procedure GetBADxListForOrder(var thisRetVal: TBADxRecord; thisOrderID: string);
...
//kt function  AllSelectedDxBlank(const Caller: smallint) : boolean;
...
//kt procedure BACopyOrder(sourceOrderList: TStringList);  //BAPHII 1.3.2
//kt procedure CopyTreatmentFactorsDxsToCopiedOrder(pSourceOrderID:string; pTargetOrderID:string); //BAPHII 1.3.2
//kt procedure CopyTreatmentFactorsDxsToRenewedOrder; //BAPHII 1.3.2
...
uses {fBALocalDiagnoses, fOrdersSign, fReview,} uCore, rCore, rPCE,uPCE,
     UBAConst, {UBAMessages,} UBACore , VAUtils;
...
begin
   ...
   //kt if tempDxNodeExists(sourceOrderID) then
   //kt   GetBADxListForOrder(thisRecord, sourceOrderID); //load data from source
...
begin
  ...
  //kt GetBADxListForOrder(retVal, thisOrderID);
...
begin
     //kt frmBALocalDiagnoses.LoadTempRec(tempDxRec, thisOrderID);
