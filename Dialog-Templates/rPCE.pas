...
begin
  ...
  //kt EncDt := Trunc(uEncPCEData.VisitDateTime);
...
var
  ...
  x: string;
begin
  //kt if (uLastLocation <> uEncLocation) then LoadEncounterForm;
...
begin
  //kt EncDT := uEncPCEData.VisitDateTime;
  Dest.Clear;
  i := SectionIndex + 1;           // first line after the section name
...
begin
  ...
  if(Result < 0) then
    begin
      //kt if Assigned(uEncPCEData) then         // may not exist yet on display of note and PCE data
      //kt   EncDT := Trunc(uEncPCEData.VisitDateTime)
...
begin
  ...
  begin
    ...
    begin
      ...
      uHasCPT.Assign(RPCBrokerV.Results);
    end;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      if idx = 0 then
      begin
        //kt if (typ = 1) or (typ = 2) then
        //kt   tCallV(TmpSL,RPC,[uEncPCEData.VisitDateTime])
        //kt else
...
function AutoCheckout(Loc: integer): boolean;
begin
  if(uLastChkOutLoc <> Loc) then begin
    uLastChkOutLoc := Loc;
