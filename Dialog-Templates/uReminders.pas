...
type
  ...
  public
    ...
    procedure TMGForgetControl(Obj : TControl);  //kt added
  ...
  protected
    ...
    procedure HandleOnGBClick(Sender: TObject); //kt added
    procedure HandleOnFieldPanelClick(Sender: TObject);  //kt added
  public
    constructor Create;
    destructor Destroy; override;
...
//    FRoot: string;
  ...
  public
    ...
    //kt function VitalType: TVitalType;
...
var
  ...
  DlgBackgroundColor : TColor = clWhite;  //kt added
...
type
  TExposedControl = class(TControl);  //kt added TEMP
      ...
      try
        if(RPCBrokerV.Results.Count > 0) then begin
          for i := 0 to RPCBrokerV.Results.Count-1 do begin
            ...
            if(idx < 0) then begin
              TempList.Add(RemID);
              if(not ProcessingInBackground) then begin
...
begin
  ...
  if(Copy(SIEN,1,1) = RemCode) then begin
    ...
    if(idx < 0) then begin
      RemindersInProcess.Notifier.BeginUpdate;
      try
        AReminder := TReminder.Create(ARemData); //kt added
        idx := RemindersInProcess.AddObject(SIEN, AReminder);
        //kt idx := RemindersInProcess.AddObject(SIEN, TReminder.Create(ARemData));
      finally
        RemindersInProcess.Notifier.EndUpdate;
      end;
    end;
  ...
  try
    ...
    if(ObjList.Count > 0) then begin
      ...
      while (i < ObjList.Count) do begin
        if(pos(ObjMarker, ObjList[i]) = 1) then begin
          obj := copy(ObjList[i], ObjMarkerLen+1, MaxInt);
          if(obj = '') then break;
          j := i + 1;
          while (j < ObjList.Count) and (pos(ObjMarker, ObjList[j]) = 0) do begin
            inc(j);
          end;
          if((j - i) > 2) then begin
            ObjTxt := '';
...
begin
  if not assigned(TMGControls) then TMGControls := TORStringList.Create; //kt added
  TempSL := GetDlgSL;
  ...
  repeat
    ...
    if(idx >= 0) then begin
      ...
      with Element do begin
        ...
        FElements[eidx] := FDlgID;  //store into .String property
        if(ElemType = etTaxonomy) then begin
       ...
       // if Piece(FRec1,U,5) <> '1' then
        ...
        if(pos('.',FDlgID)>0) then begin
          ParentID := FDlgID;
          i := length(ParentID);
          while((i > 0) and (ParentID[i] <> '.')) do begin
            dec(i);
          end;
          if(i > 0) then begin
            ...
            if(i >= 0) then begin
              ...
              if(not assigned(FParent.FChildren)) then begin
...
begin
  ...
  FreeAndNil(TMGControls);  //kt added
  inherited;
end;

procedure TReminderDialog.TMGForgetControl(Obj : TControl);
//kt added entire procedure
var i : integer;
begin
  i := TMGControls.IndexOfObject(Obj);
  if i >=0 then TMGControls.Delete(i);
end;

procedure TReminderDialog.TMGSetCBValueForControl(IEN : String; Checked : boolean);
//kt added
var i : integer;
    PtrStr, EntryStr : string;
    Ptr : Pointer;
    cb: TCPRSDialogParentCheckBox;

begin
  if not assigned(TMGControls) then exit;
  i := TMGControls.IndexOfPiece(IEN,'^',1,-1);
  if i < 0 then exit;  //no error if not found
  EntryStr := TMGControls.Strings[i];
  EntryStr := Piece(EntryStr, '^',3);  //Should be 'CB=123456789'  Number is pointer to CB
  if Piece(EntryStr,'=',1) <> 'CB' then exit;
  PtrStr := Piece(EntryStr,'=',2);
  Ptr := Pointer(StrToIntDef(PtrStr,0));
  if assigned(Ptr) then begin;
    cb := TCPRSDialogParentCheckBox(Ptr);
    cb.Checked := Checked;  //--> will triger OnClick event;
  end;  
end;


procedure TReminderDialog.TMGHighlightControl(IEN : string);
//kt added
var i : integer;
    s : string;
    Ctrl : TExposedControl;
    SetColor : TColor;
const
    HIGHLIGHT_COLOR : TColor = clYellow;
begin
  if not assigned(TMGControls) then exit;
  //first reset any prior highlighted control.
  for i := 0 to TMGControls.Count-1 do begin
    Ctrl := TExposedControl(TMGControls.Objects[i]);
    if not assigned(Ctrl) then continue;
    s := Piece(TMGControls.Strings[i],'^',4);
    if (s <> '') and (Piece(s,'=',1) = 'COLOR') then begin
      s := Piece(s, '=', 2);
      SetColor := TColor(StrToInt(s));;
    end else begin
      SetColor := DlgBackgroundColor;;
    end;
  ...
  //Now set all controls linked to IEN to highlight color.
  i := -1;
  repeat
    i := TMGControls.IndexOfPiece(IEN,'^',1,i);
    if i > -1 then begin
      Ctrl := TExposedControl(TMGControls.Objects[i]);
      if not assigned(Ctrl) then continue;
      s := TMGControls.Strings[i];
      SetPiece(s, '^', 4, 'COLOR='+IntToStr(Integer(Ctrl.Color)));  //store prior color.
      try       //kt   research why this is necessary
        Ctrl.Color := HIGHLIGHT_COLOR;
      except
        on EAccessViolation do continue;
      end;
    end;
...
var
  ...
  begin
    if(Item.ElemType = etDisplayOnly) then begin
...
begin
  if(not assigned(ReminderDialogInfo)) then begin
...
begin
  FRemData := ARemData;
  //kt  This is TReminderDialog constructor, apparently pointer is discarded
  //kt NOTE: TReminderDialog is ancestor class for TReminder
  BaseCreate;  //kt note <-- Kind of like calling 'inherited'
end;
...
begin
  Result := dtUnknown;
  for idx := low(TRemDataType) to high(TRemDataType) do begin
    if(Code = RemDataCodes[idx]) then begin
      Result := idx;
      break;
    end;
  end;
...
begin
  if (FChecked) then begin
    ...
    if(Result and (ElemType = etDisplayOnly)) then begin
      //Result := FALSE;
      if(assigned(FPrompts) and (FPrompts.Count > 0)) or
        (assigned(FData) and (FData.Count > 0)) or Result then begin
...
begin
  ...
  if(FReminder.GetDlgSL.IndexOfPieces([RPCCalled, FID, FTaxID]) < 0) then begin
...
begin //TRemDlgElement.UpdateData;
  if FHaveData then exit;
  ...
  begin
    ...
    repeat
      ...
      if (idx >= 0) and (Pieces(TempSL[idx-1],U,1,6) = Pieces(TempSL[idx],u,1,6)) then begin
        if pos(':', Piece(TempSL[idx],U,7)) > 0 then begin  //if has date ranges
          if RData <> nil then begin
            if (not assigned(RData.FActiveDates)) then begin
      ...
      if(idx >= 0) and (Pieces(TempSL[idx-1],U,1,6) <> Pieces(TempSL[idx],u,1,6)) then begin
        ...
        if(dt <> dtUnknown) and ((dt <> dtOrder) or
          ...
          ((dt <> dtMentalHealthTest) or MHTestsOK) then begin
          if(not assigned(FData)) then begin
            FData := TList.Create;
          end;
          RData := TRemData(FData[FData.Add(TRemData.Create)]);
          if pos(':',Piece(TempSL[idx],U,7)) > 0 then begin
            RData.FActiveDates := TStringList.Create;
...
//            FRoot := FRec3;
            i := idx + 1;
            ChoiceIdx := 0;
            while((i < TempSL.Count) and (TempSL.PiecesEqual(i, ['5', FID, FTaxID]))) do begin
              if (Pieces(TempSL[i-1],U,1,6) = Pieces(TempSL[i],U,1,6)) then begin
                if pos(':', Piece(TempSL[i],U,7)) > 0 then begin
                ...
                if(not assigned(FChoices)) then begin
                ...
                if pos(':',Piece(TempSL[i],U,7)) > 0 then begin
                  ...
                  FChoicesActiveDates.Insert(ChoiceIdx, ChoicesActiveDates);
                end else if assigned(FChoicesActiveDates) then begin
                  FChoicesActiveDates.Insert(ChoiceIdx, TStringList.Create);
                end;
                inc(i);
              end;
            end;
            if(assigned(FChoices)) and (FChoices.Count = 1) then begin // If only one choice just pick it
            ...
            if(dt = dtVitals) then begin
              if(Code2VitalType(Piece(FRec3,U,6)) <> vtUnknown) then begin
                if(not assigned(FPrompts)) then begin
                  FPrompts := TList.Create;
                end;
                FChoicePrompt := TRemPrompt(FPrompts[FPrompts.Add(TRemPrompt.Create)]);
  ...
  //                         RData.InternalValue + U + 'P' + U + Tmp + U + BOOLCHAR[SameL] + U + '1';
            ...
            if(dt = dtMentalHealthTest) then begin
              if(not assigned(FPrompts)) then begin
                FPrompts := TList.Create;
              end;
              FChoicePrompt := TRemPrompt(FPrompts[FPrompts.Add(TRemPrompt.Create)]);
...
//                         RData.InternalValue + U + 'P' + U + Tmp + U + BOOLCHAR[SameL] + U + '1';
                FRec4 := '4' + U + FID + U + FTaxID + U + U + BOOLCHAR[not RData.Add2PN] + U +
                         U + 'P' + U + Tmp + U + BOOLCHAR[NewLine] + U + '0';
                FData := RData;
                if ((Piece(FRec3, U, r3GAF) = '1')) and (MHDLLFound = false) then begin
                  FOverrideType := ptGAF;
    ...
    repeat
      ...
      if(idx >= 0) then begin
        ...
        if(pt <> ptUnknown) and ((pt <> ptComment) or (not FHasComment)) then begin
          if(not assigned(FPrompts)) then begin
            FPrompts := TList.Create;
          end;
          RPrompt := TRemPrompt(FPrompts[FPrompts.Add(TRemPrompt.Create)]);
          with RPrompt do begin
            FParent := Self;
            FRec4 := TempSL[idx];
            InitValue;
          end;
...
var
  ...
  begin
    if(Elem.IsChecked) then begin
      if(assigned(Elem.FPrompts)) then begin
        for i := 0 to Elem.FPrompts.Count-1 do begin
          Prompt := TRemPrompt(Elem.FPrompts[i]);
          if Prompt.Forced then begin
            try
              Prompt.SetValueFromParent(Prompt.FValue);
            except
              on E: EForcedPromptConflict do begin
                Elem.FChecked := FALSE;
      ...
      if(Elem.FChecked) and (assigned(Elem.FChildren)) then begin
        for i := 0 to Elem.FChildren.Count-1 do begin
...
begin
  if(FChecked <> Value) then begin
    MainU.MainForm.RemDlgExpandElement(FID, Value); //kt added
    FChecked := Value;
    if(Value) then begin
      GetData;
      if(FChecked and assigned(FParent)) then begin
        FParent.Check4ChildrenSharedPrompts;
        if(FParent.ChildrenRequired in [crOne, crNoneOrOne]) then begin
          for i := 0 to FParent.FChildren.Count-1 do begin
            Kid := TRemDlgElement(FParent.FChildren[i]);
            if(Kid <> Self) and (Kid.FChecked) then begin
    ...
    end else if(assigned(FPrompts) and assigned(FData)) then begin
      for i := 0 to FPrompts.Count-1 do begin
        Prompt := TRemPrompt(FPrompts[i]);
        if Prompt.Forced and (IsSyncPrompt(Prompt.PromptType)) then begin
          for j := 0 to FData.Count-1 do begin
            RData := TRemData(FData[j]);
            if(assigned(RData.FPCERoot)) then begin
              RData.FPCERoot.UnSync(Prompt);
            end;
            if(assigned(RData.FChoices)) then begin
              for k := 0 to RData.FChoices.Count-1 do begin
                if(assigned(RData.FChoices.Objects[k])) then begin
...
begin
  ...
  begin
    if((ElemType = etDisplayOnly) or FChecked) then begin
      Result := TRUE
    end else begin
...
var
  ...
  function GetPanel(const EID, AText: string; const PnlWidth: integer;
                    OwningCheckBox: TCPRSDialogParentCheckBox): TDlgFieldPanel; //in TRemDlgElement.BuildControls
  var
    idx, p: integer;
  ...
  end;  //GetPanel in TRemDlgElement.BuildControls
  ...
  begin
    MaxY := 0;
    for i := 0 to SameLineCtrl.Count-1 do begin
      C := TControl(SameLineCtrl[i]);
      if(MaxY < C.Height) then begin
        MaxY := C.Height;
      end;
    end;
    for i := 0 to SameLineCtrl.Count-1 do begin
    ...
    inc(Y, MaxY);
    if assigned(cb) and assigned(pnl) then begin
      cb.Top := pnl.Top;
    end;
    SameLineCtrl.Clear;
  end;  //NextLine in TRemDlgElement.BuildControls

  procedure ProcessLabel(Required, AEnabled: boolean;
                         AParent: TWinControl; Control: TControl);  begin
    if(Trim(Prompt.Caption) = '') and (not Required) then begin
      lblCtrl := nil
    end else begin
...
//                (sLbl as ICPRSDialogComponent).BeforeText := ScreenReaderSystem_GetPendingText;
      ...
      if Required then begin
        if assigned(Control) and Supports(Control, ICPRSDialogComponent) then begin
      ...
      TExposedControl(lblCtrl).Color := DlgBackgroundColor; //kt added
      Self.FReminder.TMGControls.AddObject(FID+'^LABEL^CB='+IntToStr(Integer(CB)),lbl); //kt added.  FID = IEN
    {  UpdateColorsFor508Compliance(lblCtrl);}
    end;
  end;  //ProcessLabel in TRemDlgElement.BuildControls
  ...
  end;  //ScreenReaderSupport in TRemDlgElement.BuildControls
  ...
  begin  //AddPrompts in TRemDlgElement.BuildControls
    ...
    try
      if(assigned(cb)) then begin
        ...
        if(cbSingleLine and (not Shared)) then begin
      ...
      for i := 0 to FPrompts.Count-1 do begin
        Prompt := TRemPrompt(FPrompts[i]);
        if not assigned(Prompt) then continue;  //kt added
...
//            if(Kid.ElemType <> etDisplayOnly) and (Kid.FChecked) then
        ...
        if(OK) then begin
          ...
          case pt of
     {case} ptComment, ptQuantity: begin
              Ctrl.edt := TCPRSDialogFieldEdit.Create(AOwner);
              Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.edt.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
     ...
     {case} ptVisitLocation, ptLevelUnderstanding,
            ...
            ptLevelSeverity, ptSkinResults, ptSkinReading: begin
              ...
              Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.cbo.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
              Ctrl.ctrl.Parent := AParent;
            ...
            end;  //ptVisitLocation, ptLevelUnderstanding, ptSeries, ptReaction, ptExamResults, ptLevelSeverity, ptSkinResults, ptSkinReading

    {case}  ptWHPapResult: begin
              if FData<>nil then begin
                if (TRemData(FData[i]).DisplayWHResults)=true then begin
                  NextLine(Y);
                  Ctrl.btn := TCPRSDialogButton.Create(AOwner);
                  Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.btn.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
                  Ctrl.ctrl.Parent := AParent;
                  ...
                  Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.WHChk.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
                  Ctrl.ctrl.Parent := AParent;
                  ProcessLabel(Prompt.Required, TRUE, Ctrl.WHChk.Parent, Ctrl.WHChk);
                  if lblCtrl is TWinControl then begin
...
//                      TExposedComponent(Ctrl.WHChk.Flbl).Caption);
                  ...
                  Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.WHChk.Check2.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
                  ...
                  Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.WHChk.Check3.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
...
//                  TExposedComponent(Ctrl.WHChk.Flbl).Caption)+4;
              ...
              Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.WHChk.Fbutton.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
              ...
              if piece(Prompt.FRec4,u,12)='1' then begin
                ...
                Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.WHChk.FPrintNow.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
    ...
    {case}  ptVisitDate: begin
              Ctrl.dt := TCPRSDialogDateCombo.Create(AOwner);
              Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.dt.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
              Ctrl.ctrl.Parent := AParent;
    ...
    {case}  ptPrimaryDiag, ptAdd2PL, ptContraindicated: begin
              Ctrl.cb := TCPRSDialogCheckBox.Create(AOwner);
              Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.cb.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
              Ctrl.ctrl.Parent := AParent;
    ...
    {case}  else begin
              if(pt = ptSubComment) then begin
                ...
                if(vt = vtPain) then begin
                  ...
                  Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.vedt.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
                  Ctrl.ctrl.Parent := AParent;
                  ...
                  if(vt in [vtTemp, vtHeight, vtWeight]) then begin
                    ...
                    Self.FReminder.TMGControls.AddObject(FID+'^'+Ctrl.vedt.LinkedCombo.ClassName+'^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
                    ...
                    if(Ctrl.vedt.LinkedCombo.ItemIndex < 0) then begin
                ...
                Ctrl.cbo.Style := orcsDropDown;
                Ctrl.cbo.Pieces := '12';
                if ActChoicesSL = nil then begin
      ...
      end;  // loop through FPrompts
      NextLine(Y);
    finally
      SameLineCtrl.Free;
    end;
  end;  //AddPrompts in TRemDlgElement.BuildControls
...
begin  //begin TRemDlgElement.BuildControls
  Result := nil;
  cb := nil;
  pnl := nil;
  ...
  if(ElemType = etDisplayOnly) then begin
    if(FText <> '') then begin
      ...
      pnl.TMGIEN := FID; //kt added
      pnl.Color := DlgBackgroundColor; //kt added
      pnl.OnClick := HandleOnFieldPanelClick;  //kt added
      Self.FReminder.TMGControls.AddObject(FID+'^PANEL^CB='+IntToStr(Integer(CB)),pnl); //kt added.  FID = IEN
      pnl.Left := X;
      pnl.Top := Y;
    ...
    cb.Color := DlgBackgroundColor; //kt added
    ...
    for i := 0 to pnl.ControlCount - 1 do begin
      //kt if (pnl.Controls[i]) is TControl then begin  //kt added block
      //kt   TExposedControl(pnl.Controls[i]).Color := clSilver; //kt TEMP!!
      //kt end;
      if ((pnl.Controls[i] is TLabel) {or (pnl.Controls[i] is TVA508StaticText)}) and
        not (fsUnderline in TLabel(pnl.Controls[i]).Font.Style) then begin
...
//      end;
      ...
      if(not EnableChildren) then begin
        gb.Font.Color := DisabledFontColor;
      end;
      //kt gb.Color := clSilver; //clMaroon; //kt added
      //kt mod start ---------------
      //if Pos(TMG_DYNAMIC_IEN_TAG,Self.FID)=1 then begin
      //  gb.Tag := StrToInt(MidStr(Self.FID,Length(TMG_DYNAMIC_IEN_TAG)+1, Length(Self.FID))); //IEN of parent object
      //end else begin
      //  gb.Tag := StrToInt(Self.FID); //IEN of parent object
      //end;
      //kt mod end ------------------
      gb.Tag := StrToInt(Self.FID); //IEN of parent object
      gb.OnClick := HandleOnGBClick; //kt added
      Self.FReminder.TMGControls.AddObject(FID+'^GRPBOX^CB='+IntToStr(Integer(CB)),gb); //kt added.  FID = IEN
...
procedure TRemDlgElement.HandleOnGBClick(Sender: TObject);
//kt added entire procedure
var IEN : string;
begin
  if Sender is TGroupBox then begin
    IEN := IntToStr(TGroupbox(Sender).Tag);
    MainForm.RemDlgDisplayElement(IEN);
  end;
end;
...
var DlgFldPanel : TDlgFieldPanel;
    CB : TORCheckBox;
begin
//  if TFieldPanel(Sender).Focus then
  //ktTORCheckBox(TDlgFieldPanel(Sender).Tag).Checked := not FChecked;
  DlgFldPanel := TDlgFieldPanel(Sender);
  HandleOnFieldPanelClick(Sender); //kt added
  CB := TORCheckBox(DlgFldPanel.Tag);
  //kt temp CB.Checked := not FChecked;
end;

procedure TRemDlgElement.HandleOnFieldPanelClick(Sender: TObject);
//kt added entire procedure
var DlgFldPanel : TDlgFieldPanel;
begin
  if not (Sender is TDlgFieldPanel) then exit;
  DlgFldPanel := TDlgFieldPanel(Sender);
  {
  MessageDlg('You Clicked -- message box is in uReminders:TRemDlgElement.TMGFieldPanelOnClick'+#13#10+
             'Sender is '+Sender.ClassName + ', IEN = '+ DlgFldPanel.TMGIEN,
             mtInformation,[mbOK],0);  //kt added
  }           
  MainForm.RemDlgDisplayElement(DlgFldPanel.TMGIEN);
end;
...
var
  ...
  procedure AddPrompt(Prompt: TRemPrompt; dt: TRemDataType; var x: string); //in TRemData.AddData(
  var
    pt: TRemPromptType;
    pnum: integer;
    Pdt: TRemDataType;
  ...
  end;  //AddPrompt in TRemData.AddData(

  procedure Add(Str: string; Root: TRemPCERoot);  //in TRemData.AddData
  var
    i, Qty: integer;
  ...
  begin
    ...
    Primary := FALSE;
    if(assigned(FParent) and assigned(FParent.FPrompts) and (PCECat = pdcDiag)) then begin
            ...
            if (Sender is TWHCheckBox) then begin
              ...
              if whCKB.Checked = true then begin
                ...
                if whCKB.Caption ='In-Person' then begin
                  ...
                  else if Pos('I',FParent.WHResultNot)=0 then begin
                ...
                if whCKB.Caption ='Phone Call' then begin
                  ...
                  else if Pos('P',FParent.WHResultNot)=0 then begin
                ...
                if whCKB.Caption ='Letter' then begin
                  for i:=1 to Length(WHValue) do begin
                    if WHValue1='' then begin
                      if (WHValue[i]<>'L') and (WHValue[i]<>':') then begin
                        WHValue1 := WHValue[i];
                      end;
                    end else if (WHValue[i]<>'L') and (WHValue[i]<>':') then begin
                ...
                if whCKB.Caption ='In-Person' then begin
                  for i:=1 to Length(WHValue) do begin
                    ...
                    end else if (WHValue[i]<>'I') and (WHValue[i]<>':') then begin
                      WHValue1 := WHValue1+':'+WHValue[i];
                    end;
                  end;
                end;
                if whCKB.Caption ='Phone Call' then begin
                  for i:=1 to Length(WHValue) do begin
                    ...
                    end else if (WHValue[i]<>'P') and (WHValue[i]<>':') then begin
                      WHValue1 := WHValue1+':'+WHValue[i];
                    end;
                  end;
                end;
              ...
              if (((Sender as TORCheckBox).Caption = 'Print Now') and
                  ...
                  ((Sender as TORCheckBox).Checked = false) then begin
                FParent.WHPrintDevice := '';
                FPrintNow :='0';
              end;
            end;
