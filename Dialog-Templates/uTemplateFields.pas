...
uses
  rTemplates, ORCtrls, mTemplateFieldButton, dShared, uConst, uCore, rCore,
  Windows, VAUtils {, VA508AccessibilityManager, VA508AccessibilityRouter};
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //kt start mod
        for tmg := 0 to AList.Count-1 do begin
          InactiveList.Add(AList[tmg]);
        end;
        //kt end mod
        //kt FastAddStrings(InactiveList, AList);
      end;
      if(AList.Count > 0) then
      begin
...
begin
    ...
    case FFldType of
      dftEditBox:
        begin
          ...
          //kt UpdateColorsFor508Compliance(edt, TRUE);
          ctrl := edt;
        end;
      ...
      dftComboBox:
        begin
          ...
          //kt UpdateColorsFor508Compliance(cbo, TRUE);
          ctrl := cbo;
        end;
      ...
      dftButton:
        begin
          ...
          //kt UpdateColorsFor508Compliance(btn);
          ctrl := btn;
        end;
  ...
  //              cb.Height := ht;
              ...
              //kt UpdateColorsFor508Compliance(cb);
              inc(Index);
      ...
      dftDate:
        begin
          ...
          begin
            ...
            //kt UpdateColorsFor508Compliance(dcbo, TRUE);
            ctrl := dcbo;
          end
          else
          begin
            ...
            //kt UpdateColorsFor508Compliance(dbox, TRUE);
            ctrl := dbox;
          end;
        end;
      ...
      dftNumber:
        begin
          ...
          //kt UpdateColorsFor508Compliance(pnl, TRUE);
          //CQ 17597 wat
      ...
      dftHyperlink, dftText:
        begin
          ...
          lbl.Tag := CtrlID;
          //kt UpdateColorsFor508Compliance(lbl);
          ctrl := lbl;
        end;
      ...
      dftWP:
        begin
          ...
          //kt UpdateColorsFor508Compliance(re, TRUE);
          ctrl := re;
        end;
    end;
...
begin
  ...
  begin
    ...
    Result := FLocked;
    if(not FLocked) then
      //kt ShowMsg('Template Field ' + FFldName + ' is currently being edited by another user.');
...
begin
  ...
  if(FControls.Count > 1) then begin
    ...
    //kt if not ScreenReaderSystemActive then
  ...
  //kt UpdateColorsFor508Compliance(FPanel, TRUE);
  idx := 0;
...
const
  ...
  procedure Add2TabOrder(ctrl: TWinControl); //TTemplateDialogEntry.GetPanel,Add2TabOrder
  begin
    ctrl.TabOrder := TabOrdr;
    inc(TabOrdr);
  end;

  function StripSRCode(var txt: string; code: string; len: integer): integer;  //TTemplateDialogEntry.GetPanel,Add2TabOrderStripSRCode
  begin
    Result := pos(code, txt);
    if Result > 0 then
    begin
  ...
  procedure DoLabel(Atxt: string);  //TTemplateDialogEntry.GetPanel,DoLabel
  var
    ctrl: TControl;
    //kttempLbl: TVA508ChainedLabel;
  ...
  procedure Init;  //TTemplateDialogEntry.GetPanel,Init
  var
    lbl : TLabel;
  begin
    if(FFirstBuild) then
  ...
  procedure Text508Work;   //TTemplateDialogEntry.GetPanel,Text508Work
  var
    ContinueCode: boolean;
  begin
    if StripCode(txt, SR_BREAK) then
    begin
      //kt ScreenReaderSystem_Stop;
      nLbl := nil;
      sLbl := nil;
    end;
  ...
  procedure Ctrl508Work(ctrl: TControl);  //TTemplateDialogEntry.GetPanel,Ctrl508Work
  var
    lbl: TCPRSTemplateFieldLabel;
  begin
    ...
    begin
      ...
      begin
        //kt ScreenReaderSystem_CurrentLabel(lbl);
        //kt ScreenReaderSystem_AddText(lbl.Caption);
      end
      else
      begin
        //kt lbl.TabStop := FALSE;
        //kt ScreenReaderSystem_Stop;
      end;
    end
    else
    begin
      ...
      if Supports(ctrl, ICPRSDialogComponent) then begin
        //kt ScreenReaderSystem_CurrentComponent(ctrl as ICPRSDialogComponent);
      end;
    end;
    sLbl := nil;
    nLbl := nil;
  ...
  procedure NextLine;  //TTemplateDialogEntry.GetPanel,NextLineNextLine
  begin
    if(MaxX < x) then
      MaxX := x;
...
begin  //Begin TTemplateDialogEntry.GetPanel(MaxLen: integer; AParent: TWinControl; OwningCheckBox: TCPRSDialogParentCheckBox): TDlgFieldPanel;
  ...
  if(FFirstBuild or (FPanel.Width <> MaxLen)) then begin
    ...
    for i := 0 to FControls.Count-1 do begin
      ...
      if StripCode(txt,EOL_MARKER) then begin
        if((x <> 0) or LastLineBlank) then begin
          NextLine;
        end;
        LastLineBlank := (txt = '');
      end;
      if(txt <> '') then begin
        ...
        if(assigned(ctrl)) then begin
          ...
          if(x > 0) then begin
            if (x < MaxLen) and (Ctrl is TORCheckBox) and (TORCheckBox(Ctrl).StringData = NewLine) then begin
              x := MaxLen;
            end;
            if((ctrl.Width + x + ind) > MaxLen) then begin
              NextLine;
            end;
          end;
          inc(x,ind);
          ...
          Ctrl.Top := y;
          inc(x, Ctrl.Width + 4);
          if yinc <= Ctrl.Height then begin
            yinc := Ctrl.Height + 2;
          end;
          if (x < MaxLen) and ((Ctrl is TRichEdit) or
             ((Ctrl is TLabel) and (pos(CRLF, TLabel(Ctrl).Caption) > 0))) then begin
            x := MaxLen;
          end;
        end;
      end;
