...
type
  TfrmNotes = class(TfrmHSplit)
    ...
    //kt imgLblNotes: TVA508ImageListLabeler;
    //kt imgLblImages: TVA508ImageListLabeler;
  ...
  public
    ...
    function AllowContextChange(var WhyNot: string): Boolean; //kt override;
    procedure ClearPtData; //kt override;
    procedure DisplayPage; //kt override;
    procedure RequestPrint; //kt override;
    //kt procedure RequestMultiplePrint(AForm: TfrmPrintList);
    procedure SetFontSize(NewFontSize: Integer); //kt override;
    ...
    procedure ForceReminderShow;  //kt added
  published
...
begin
  ...
  begin
    ...
    if ItemIEN > 0 then begin
      //kt PrintNote(ItemIEN, MakeNoteDisplayText(Items[ItemIndex]))
    end else begin
...
begin
  inherited;
  //kt frmFrame.mnuChartTabClick(Sender);
end;
...
procedure TfrmNotes.ForceReminderShow;
//kt added this entire function
//Copied and modified from DisplayPCE
var
...
begin
  ...
  with uPCEEdit do begin
    ...
    if(NoPCE and memPCEShow.Visible) then begin
      memPCEShow.Lines.Insert(0, TX_NOPCE);
    end;
    memPCEShow.SelStart := 0;
    EnableList := [odReminders];
    ShowList := [odReminders];
  ...
  with lstNotes do if ItemIndex = EditingIndex then begin
    with uPCEEdit do begin
      ...
      if(InteractiveRemindersActive) then begin
        ...
        end else if FutureEncounter(uPCEEdit) then begin
          EnableList := [odTemplates];
...
begin
  ...
  begin
    ...
    //kt if IsConsultTitle(Title) and (PkgIEN = 0) then Result := True;
    //kt if IsSurgeryTitle(Title) and (PkgIEN = 0) then Result := True;
...
begin
  //kt if frmFrame.Timedout then Exit;
  ...
  try
    ...
    with FEditNote do if (PkgIEN > 0) and (PkgPtr = PKG_CONSULTS) then begin
      //ktHaveRequired := LockConsultRequest(PkgIEN);
      HaveRequired := true; //kt
    end;
    ...
    begin
      ...
      begin
        ...
        //kt with FEditNote do if (PkgIEN > 0) and (PkgPtr = PKG_CONSULTS) then UnlockConsultRequest(0, PkgIEN);
    ...
    begin
      ...
      memNewNote.Lines.Assign(TmpBoilerPlate); //kt
...
begin
  ...
  if HaveRequired then
    with FEditNote do
      if (PkgIEN > 0) and (PkgPtr = PKG_CONSULTS) then begin
        //kt HaveRequired := LockConsultRequest(PkgIEN);
        HaveRequired := true; //kt
      end;
  if HaveRequired then
  begin
...
begin
  ClearEditControls;
  //kt if not LockConsultRequestAndNote(lstNotes.ItemIEN) then Exit;
...
var
  ...
  begin
    ...
    //kt QuickCopyWith508Msg(BoilerText, memNewNote);
    memNewNote.Lines.Assign(BoilerText); //kt
    UpdateNoteAuthor(DocInfo);
    FChanged := False;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        1: begin
             ExecuteTemplateOrBoilerPlate(BoilerText, FEditNote.Title, ltTitle, Self, 'Title: ' + FEditNote.TitleName, DocInfo);
             //kt QuickAddWith508Msg(BoilerText, memNewNote);  // append
             memNewNote.Lines.AddStrings(BoilerText); //kt added  // append
             UpdateNoteAuthor(DocInfo);
           end;
...
var
  ErrMsg: string;
begin
  //kt if fFrame.frmFrame.DLLActive = true then Exit;
...
var
  ...
  //kt SearchCtxt: TSearchContext; // Text Search CQ: HDS00002856
...
begin
  ...
  NC_CUSTOM:     begin
                   if Sender is TMenuItem then
                     begin
                       //kt SelectTIUView(Font.Size, True, FCurrentContext, uTIUContext);
...
begin
  ...
  begin
    ...
    //kt frmFrame.DisplayEncounterText;
  end;
  if Encounter.NeedVisit then
  begin
...
begin
  ...
  //kt LockConsultRequestAndNote(lstNotes.ItemIEN);
...
begin
  inherited;
  MessageDlg('Signing notes not allowed in VistA-Config',mtWarning,[mbOK],0); //kt
  (*  //kt
{  if lstNotes.ItemIndex = EditingIndex then                //v22.12 - RV
  ...
  begin
    ...
    //kt if frmFrame.Closing then exit;
...
begin
  ...
  //kt UnlockConsultRequest(IEN);
  // GE 14926; added if (AnIndex> -1) to by pass LoadNotes when creating on narking Allerg Entered In error.
  if (AnIndex > -1) and (AnIndex = lstNotes.ItemIndex) {kt and (not frmFrame.ContextChanging)} then begin
    LoadNotes;
...
begin
  inherited;
  if {kt frmFrame.TimedOut  and } (EditingIndex <> -1) then
  begin
    FSilent := True;
...
begin
  inherited;
  //ktEditTemplates(Self, TRUE, FEditCtrl.SelText);
end;
          ...
          begin
            ...
            if FDocList.Count>0 then
              for x := FDocList.Count-1 downto 1 do begin;  // Don't do 0, because it's informational
                ...
                //kt frmSearchStop.lblSearchStatus.Caption := lblNotes.Caption;
                //kt frmSearchStop.lblSearchStatus.Repaint;
                lblNotes.Repaint;
...
begin
  ...
  begin
    if (Screen.ActiveControl = lvNotes) or
      ...
      //kt frmFrame.pnlPatient.SetFocus
    else
    if (Screen.ActiveControl = tvNotes) then
      //kt FindNextControl( frmFrame.pnlPatient, False, True, False).SetFocus;
  end;
end;

procedure TfrmNotes.frmFramePnlPatientExit(Sender: TObject);
begin
  //kt FOldFramePnlPatientExit(Sender);
  if TabIsPressed or ShiftTabIsPressed then
  begin
    if (Screen.ActiveControl = lvNotes) or
        (Screen.ActiveControl = memNote) then
...
begin
  inherited;
  //kt frmFrame.pnlPatient.OnExit := FOldFramePnlPatientExit;
...
begin
  inherited;
  //kt FOldFramePnlPatientExit := frmFrame.pnlPatient.OnExit;
  //kt frmFrame.pnlPatient.OnExit := frmFramePnlPatientExit;
...
begin
  inherited;
  //kt frmFrame.ViewInfo(Sender);
end;

procedure TfrmNotes.mnuViewInformationClick(Sender: TObject);
begin
  inherited;
  mnuViewDemo.Enabled := false; //kt frmFrame.pnlPatient.Enabled;
  mnuViewVisits.Enabled := false; //kt frmFrame.pnlVisit.Enabled;
  mnuViewPrimaryCare.Enabled := false; //kt frmFrame.pnlPrimaryCare.Enabled;
  mnuViewMyHealtheVet.Enabled := false; //kt not (Copy(frmFrame.laMHV.Hint, 1, 2) = 'No');
  mnuInsurance.Enabled := false; //kt not (Copy(frmFrame.laVAA2.Hint, 1, 2) = 'No');
  mnuViewFlags.Enabled := false; //kt frmFrame.lblFlag.Enabled;
  mnuViewRemoteData.Enabled := false; //kt frmFrame.lblCirn.Enabled;
  mnuViewReminders.Enabled := false; //kt frmFrame.pnlReminders.Enabled;
  mnuViewPostings.Enabled := false; //kt frmFrame.pnlPostings.Enabled;
end;
...
begin
  if (not pnlWrite.Visible) {kt or uInit.TimedOut} then exit;

  {kt if (frmFrame.WindowState = wsMaximized) then
    idx := GetSystemMetrics(SM_CXFULLSCREEN)
  else   }
    idx := GetSystemMetrics(SM_CXSCREEN);
  {kt if idx > frmFrame.Width then
    idx := frmFrame.Width;  }
...
initialization
  //kt SpecifyFormIsNotADialog(TfrmNotes);
